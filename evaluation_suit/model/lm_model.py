...
class HuggingFaceAutoLM(BaseLM):
    ...
    def __init__(
        ...
        model_kwargs = {}
        if use_accelerate:
            model_kwargs = utils._get_accelerate_args(
                low_cpu_mem_usage,
                device_map_option,
                max_memory_per_gpu,
                max_cpu_memory,
        ...
        self.model = self._create_auto_model(
            ...
            revision=revision,
            subfolder=subfolder,
            torch_dtype=utils._get_dtype(dtype, self._config),
            ...
            if transformers.__version__ >= "4.30.0":
                ...
                if load_in_4bit:
                    ...
                    if bnb_4bit_compute_dtype:
                        model_kwargs["bnb_4bit_compute_dtype"] = utils._get_dtype(
...
class AutoCausalLM(HuggingFaceAutoLM):
    ...
    def _model_generate(
        ...
        if isinstance(generations, transformers.generation.utils.GenerateBeamDecoderOnlyOutput) \
            or isinstance(generations, transformers.generation.utils.GenerateDecoderOnlyOutput):
            generations = generations.sequences

        generations = utils.select_continuation_from_batch_left_padding(
            generations, max_context_size=inputs["input_ids"].size(1)
        )

        # generated multiple (N) sequences (i.e. beam search with N return_sequences)
        # of size (batch_size*num_return_sequences, sequence_length)
        if generations.size(0) != input_ids.size(0):
            # to size (batch_size, num_return_sequences, sequence_length)
