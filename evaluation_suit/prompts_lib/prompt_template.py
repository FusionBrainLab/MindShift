...
def return_prompt_wrapper(
    ...
    def wrapper(*args: Any, **kwargs: Any) -> Union[Any, str]:
        r"""Wrapper function that performs the conversion to :obj:`PromptTemplate`
            instance.
...
class PromptTemplate(str):
    r"""
    A class that represents a text prompt. This class
    extends the built-in `str` class to provide a property for retrieving
    ...
    def format(self, *args: Any, **kwargs: Any) -> 'PromptTemplate':
        r"""
        ...
        Returns:
            PromptTemplate: A new :obj:`TextPrompt` object with the format string
                replaced with the formatted string.
        """
        default_kwargs = {key: '{' + f'{key}' + '}' for key in self.key_words}
        default_kwargs.update(kwargs)
        return PromptTemplate(super().format(*args, **default_kwargs))
...
def get_prompt_template_key_words(template: str) -> Set[str]:
    ...
    Example:
        ...
        {'name', 'status'}
    """
    return OrderedSet(re.findall(r'{([^}]*)}', template))